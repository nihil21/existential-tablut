...
def printOtherPredictions(predFrom, predTo, labelFrom, labelTo):
    dictPredFrom = {}
...
def generateMove(modelFrom, modelTo, labelFrom, labelTo, boardInfo, dictFromMoves, turn, lock):
    # generate the two parts of the move using the two neural networks provided

    # enable debug print
    debug = False

    board = encodeBoard(boardInfo["board"])
...
def getPoints(finalState, playerColor):
    if finalState == "DRAW":
        return 1
...
def get_move_point(result, total_move):
    if result in [3, 2]:
        return - total_move
...
def connectAndPlay(modelFrom, modelTo, labelFrom, labelTo, playerName, playerColor, port, championship, lock,
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while True:
            ...
            if (boardInfo["turn"] == "WHITE" and playerColor == "W") or (
                ...
                if not baseline_player:
                    move = generateMove(modelFrom, modelTo, labelFrom, labelTo, boardInfo, dictFromMoves, turn, lock)
                else:
                    print_debug = False
        ...
        if baseline_player and getPoints(boardInfo["turn"], playerColor) in [0, -1]:
            if playerColor == "W":
        ...
        championship.calculate(playerName, playerColor == "W", getPoints(boardInfo["turn"], playerColor), get_move_point(getPoints(boardInfo["turn"], playerColor), total_move))
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while True:
            ...
            if (boardInfo["turn"] == "WHITE" and player == "W") or (boardInfo["turn"] == "BLACK" and player == "B"):
                # player choose move
                if not baseline_player:
                    move = generateMove(modelFrom, modelTo, labelFrom, labelTo, boardInfo, dictFromMoves,
