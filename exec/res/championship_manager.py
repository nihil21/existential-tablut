...
class Score:
    ...
    def points(self):
        ...
        return 3 * self.deserved_win + 2 * self.fail_win + 1 * self.draw + 0 * self.deserved_lose + -1 * self.fail_lose

    def __repr__(self):
        return "[DW: " + str(self.deserved_win) + ", FW: " + str(self.fail_win) + ", D: " + str(self.draw) + ", DL:" + str(self.deserved_lose) + ", FL:" + str(self.fail_lose) + "]"
...
class Network:
    ...
    def calculate(self, is_white, result):
        # Enter critical section
        with self.lock:
            # Do critical work
            if is_white == 1:
                self.score_white.calculate(result)
            elif is_white == 0:
                self.score_black.calculate(result)
            else:
...
class Championship:
    ...
    def calculate(self, net_name, is_white, result):
        found = 0
        for net in self.networks:
            if net.name == net_name:
                net.calculate(is_white, result)
                found = 1
        if found == 0:
    ...
    def white_with_points(self):
        result_dict = {}
        for net in self.networks:
            result_dict[net.name] = net.score_white.points()
        return result_dict

    def black_with_points(self):
        result_dict = {}
        for net in self.networks:
...
# useful for debugging and bestemming
def test():
